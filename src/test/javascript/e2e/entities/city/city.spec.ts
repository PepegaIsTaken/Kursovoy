...
describe('City e2e test', () => {
    ...
    it('should load Cities', async () => {
        ...
        expect(await cityComponentsPage.getTitle()).to.eq('airTrafficApp.city.home.title');
    });

    it('should load create City page', async () => {
        await cityComponentsPage.clickOnCreateButton();
        cityUpdatePage = new CityUpdatePage();
        expect(await cityUpdatePage.getPageTitle()).to.eq('airTrafficApp.city.home.createOrEditLabel');
    ...
    it('should create and save Cities', async () => {
        ...
        await promise.all([
            ...
            cityUpdatePage.setCountryInput('country'),
            cityUpdatePage.setPopulationInput('5')
        ]);
        expect(await cityUpdatePage.getNameInput()).to.eq('name');
        expect(await cityUpdatePage.getCountryInput()).to.eq('country');
        expect(await cityUpdatePage.getPopulationInput()).to.eq('5');
    ...
    it('should delete last City', async () => {
        ...
        expect(await cityDeleteDialog.getDialogTitle()).to.eq('airTrafficApp.city.delete.question');
